...
function dataBind(model, initialState){
	if(typeof initialState !== 'undefined' && initialState == true){
		model.each(function(){
...
function bindValues(value,binder){
	if(binder.is("input[type=text], textarea")){
    	binder.val(value);
  	}

	if(binder.is("input[type=checkbox]")){
    	binder.val(value).prop('checked', value);
  	}
	...
	if(binder.not("input,textarea,select")){
		binder.html(value);
	}
}
...
function newFieldValues(id,type,vals) {
    ...
    'help', 'required', 'duplicates', 'readonly', 'disabled', 'disabledInsert', 'disabledUpdate', 'publicRelease',
	...
	switch(type) {
		...
		case 'text':
            var textHiddenFields = ['externalUpdateForm', 'externalUpdateForm', 'min', 'max', 'step', 'format'];
		...
		case 'file':
            var fileHiddenFields = [
                ...
                'border', 'borderHeight', 'borderWidth', 'borderColor', 'thumbnail', 'convertAudio', 'bitRate', 'audioFormat', 'convertVideo',
