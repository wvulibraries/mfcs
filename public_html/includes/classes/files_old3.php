...
class files {
	...
	private static function combineFiles($originalFiles, $originalsFilepath, $assetsID, $tmpDir, $options) {
		...
		try {
			...
			foreach ($originalFiles as $filename) {
				...
				self::processOcrAndGeneratePdf($baseFilename, $saveBase, $filename, $gsTemp);
	...
	private static function processOcrAndGeneratePdf($baseFilename, $saveBase, $originalFile, $gsTemp) {
        ...
        $cleanedJpgFile = $baseFilename . "_cleaned.jpg"; // File after running textcleaner
        ...
        if (file_exists($jpgFile)) {
            ...
            // If a new-line char is in the output, assume it's an error
            if (strpos(trim($_exec), "\n") !== FALSE) {
                errorHandle::warningMsg("Textcleaner failed for " . basename($jpgFile) . ". Continuing with original&hellip;");
                $cleanedJpgFile = $jpgFile; // Use the original if textcleaner fails
            }
            ...
            if (trim($_exec) !== '') {
                if (strpos($_exec, 'Warning:') !== FALSE) {
                    ...
                    errorHandle::errorMsg("Failed to Create PDF: " . basename($jpgFile, ".jpg") . ".pdf");
                    throw new Exception("hocr2pdf Error: " . $_exec);
                }
            }
            ...
            // Delete the cleaned JPG file if it was created
            if ($cleanedJpgFile !== $jpgFile && file_exists($cleanedJpgFile)) {
                unlink($cleanedJpgFile);
            }
        } else {
