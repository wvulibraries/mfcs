...
class files {
	...
	public static function processObjectFiles($assetsID, $options) {
		...
		try {
			if (self::shouldCombineFiles($options)) {
				$return['combine'] = self::combineFiles($originalFiles, $originalsFilepath, $assetsID);
			}
			...
			foreach ($originalFiles as $filename) {
				...
				if (self::shouldConvertFiles($options)) {
					$return['processed'][] = self::processFile($originalFile, $filename, $assetsID, $options);
				}
				...
				if (self::shouldCreateOCRFile($options)) {
					$return['ocr'][] = self::createOCRTextFile($originalFile, $assetsID, $filename);
				}
				...
				if (self::shouldConvertVideo($options)) {
					$return['video'][] = self::convertVideo($assetsID, $filename, $originalFile, $options);
				}
	
				if (self::shouldCreateVideoThumbnails($options)) {
					$return['videoThumbs'][] = self::createVideoThumbs($assetsID, $filename, $originalFile, $options);
				}
			}
		} catch (Exception $e) {
	...
	private static function combineFiles($originalFiles, $originalsFilepath, $assetsID) {
		...
		foreach ($originalFiles as $filename) {
			...
			$baseFilename = $tmpDir . DIRECTORY_SEPARATOR . $_filename['filename'];
			...
			if ($_exec === null) {
				$errors[] = "Failed to convert image to JPG: " . basename($filename);
			...
			if ($_exec === null) {
				$errors[] = "Failed to create PDF: " . basename($filename);
			}
	
			// Clean up converted JPG file
			unlink($baseFilename . ".jpg");
		}
	
		$combinedPdfPath = self::getSaveDir($assetsID, 'combine') . "combined.pdf";
		...
		if ($_exec !== null) {
			$errors[] = "GhostScript Error: $_exec";
		}
	...
	private static function processFile($originalFile, $filename, $assetsID, $options) {
		$image = new Imagick();
		...
		if (isset($options['grayscale']) && str2bool($options['grayscale'])) {
			$image->setImageColorspace(Imagick::COLORSPACE_GRAY);
		}
	
		// Add more processing based on other options
	
		// Save the processed image
		$processedFilename = $filename . '_processed.' . strtolower($image->getImageFormat());
		$processedFilePath = self::getSaveDir($assetsID, 'processed') . $processedFilename;
		$image->writeImage($processedFilePath);
		$image->clear();
		$image->destroy();
		...
		return array(
			...
			'type'   => self::getMimeType($processedFilePath),
			'errors' => '',
		);
	}	
	...
	public static function createOCRTextFile($originalFile, $assetsID, $filename){
		...
		$_exec = shell_exec(sprintf('convert %s %s 2>&1',
			...
			escapeshellarg($baseFilename.".jpg")
		));
	...
	public static function convertImage($image, $options, $assetsID, $filename){
		...
		} elseif ($options['convertWidth'] > 0 || $options['convertHeight'] > 0) {
