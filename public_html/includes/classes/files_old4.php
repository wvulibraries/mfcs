...
class files {
	...
	private static function combineFiles($originalFiles, $originalsFilepath, $assetsID, $tmpDir, $options) {
		...
		try {
			...
			foreach ($originalFiles as $filename) {
				...
				// Read and handle the content of the HTML file to check for errors
				self::processHtmlFile($htmlFile);
    ...
    private static function processHtmlFile($htmlFile) {
        // Read and handle the content of the HTML file to check for errors
        if (file_exists($htmlFile)) {
            ...
            if (!empty($errors)) {
                ...
                $cleanedHtmlContent = preg_replace('/<\?xml[^>]*>/', '', $htmlContent); // Remove problematic XML tags
                // Additional common fixes can be applied here as needed

                // Save the fixed content back to the HTML file
                file_put_contents($htmlFile, $cleanedHtmlContent);
            }
        } else {
