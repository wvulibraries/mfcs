...
class locks {
    ...
    public static function is_locked($objectID, $type) {
        ...
        $sql = sprintf("SELECT COUNT(*) as lock_count, MAX(`locks`.ID) as ID, `users`.`username` as `username`, MAX(`date`) as `date` 
                        FROM `locks` 
                        LEFT JOIN `users` ON `locks`.`user`=`users`.`ID` 
                        WHERE `type`='%s' AND `typeID`='%s' 
                        GROUP BY `locks`.`type`, `locks`.`typeID`, `users`.`username`",
            mfcs::$engine->openDB->escape($type),
            mfcs::$engine->openDB->escape($objectID)
        );
        $sqlResult = mfcs::$engine->openDB->query($sql);
