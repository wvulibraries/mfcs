...
class files {
	...
	public static function processObjectFiles($assetsID, $options) {
		...
		try {
			// if combin files is checked or ocr is checked, we can process all tif files into jpgs
			// for combine and ocr instead of it doing it twice as the old way did.
			if (self::shouldCombineFiles($options) || self::shouldOCRFiles($options)) {
				self::convertTiffsToJPGs($originalFiles, $originalsFilepath, $assetsID);
			}
			...
			// If ocr files is checked, create all the OCR files
			if (self::shouldOCRFiles($options)) {
				$return['ocr'] = self::createOCRFiles($originalFiles, $originalsFilepath, $assetsID, $options);
			}

			// Cleanup temporary jpg files
			self::cleanupTemporaryJPGs($originalFiles);
	...
	public static function createOCRTextFile($originalFile, $assetsID, $filename){
		...
		try {
			...
			$filePath = $saveDir . DIRECTORY_SEPARATOR . $baseFilename . '.txt';
	...
	private static function createOCRFiles($originalFiles, $originalsFilepath, $assetsID, $options) {
		foreach ($originalFiles as $filename) {
			$originalFile     = $originalsFilepath.DIRECTORY_SEPARATOR.$filename;
			$_filename        = pathinfo($originalFile);
			$filename         = $_filename['filename'];
	...
	private static function convertTiffsToJPGs($originalFiles) {
		foreach ($originalFiles as $filename) {
	...
	private static function cleanupTemporaryJPGs($originalFiles) {
		foreach ($originalFiles as $filename) {
			$baseFilename = $tmpDir.DIRECTORY_SEPARATOR.$filename;
			unlink($baseFilename.".jpg");
		}
	...
	private static function generateTemporaryImages() {
