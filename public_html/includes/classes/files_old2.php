...
class files {
	...
	public static function createOCRTextFile($tmpDir, $assetsID, $filename){
		...
		// Create a temporary jpg file of the original file with Fred's preprocessing
		$cleanedJpgFile = $baseFilename . "_cleaned.jpg";
		$_exec = shell_exec(sprintf(
			'convert \( %s -colorspace gray -type grayscale -contrast-stretch 0 \) ' .
			...
			'-deskew 40%% -sharpen 0x1 \ %s 2>&1',
			escapeshellarg($filename),
			escapeshellarg($cleanedJpgFile)
		));
	
		// If a new-line char is in the output, assume it's an error
		if (strpos(trim($_exec), "\n") !== FALSE) {
			error_log("Preprocessing failed for $filename. Continuing with original&hellip;");
			$cleanedJpgFile = $filename; // Use the original if preprocessing fails
		}
	
		// New TesseractOCR object
		$tesseract = new TesseractOCR($cleanedJpgFile);
	...
	private static function combineFiles($originalFiles, $originalsFilepath, $assetsID, $tmpDir, $options) {
		...
		try {
			...
			foreach ($originalFiles as $filename) {
				...
				self::processOcrAndGeneratePdf($baseFilename, $saveBase, $originalFile, $gsTemp);
			}			
	...
	private static function processOcrAndGeneratePdf($baseFilename, $saveBase, $originalFile, $gsTemp) {
		...
		if (file_exists($jpgFile)) {
            ...
            // If a new-line char is in the output, assume it's an error
            if (strpos(trim($_exec), "\n") !== FALSE) {
                errorHandle::warningMsg("Preprocessing failed for " . basename($jpgFile) . ". Continuing with original&hellip;");
                $cleanedJpgFile = $jpgFile; // Use the original if preprocessing fails
            }

            // Perform OCR using Tesseract on the preprocessed image
            $_exec = shell_exec(sprintf(
                'tesseract %s %s -l eng %s 2>&1',
                escapeshellarg($cleanedJpgFile), // input.ext
                escapeshellarg($baseFilename), // output without extension (Tesseract will add .hocr)
			...
			// Read and log the content of the HTML file to check for errors
			if (file_exists($htmlFile)) {
				$htmlContent = file_get_contents($htmlFile);
				errorHandle::newError("HTML Content: " . $htmlContent, errorHandle::DEBUG);
				...
				if (!empty($errors)) {
					foreach ($errors as $error) {
						errorHandle::newError("HTML Parse Error: " . $error->message, errorHandle::DEBUG);
					}
	
					// Attempt to fix common issues
					$htmlContent = preg_replace('/<\?xml[^>]*>/', '', $htmlContent); // Remove problematic XML tags
					// Additional common fixes can be applied here as needed
	
					// Re-log the cleaned HTML content
					errorHandle::newError("Cleaned HTML Content: " . $htmlContent, errorHandle::DEBUG);
	
					// Save the fixed content back to the HTML file
					file_put_contents($htmlFile, $htmlContent);
				}
			} else {
				errorHandle::warningMsg("File not found for inspection: " . $htmlFile);
			}
	
			// Create an OCR'd pdf of the converted file
			$_exec = shell_exec(sprintf(
				'hocr2pdf -i %s -o %s -s < %s 2>&1',
				escapeshellarg($jpgFile), // input.ext
				escapeshellarg($pdfFile), // output.pdf
				escapeshellarg($htmlFile) // input.html
			));
			...
			// Verify the PDF file exists before adding to gsTemp
			if (file_exists($pdfFile)) {
				// Add this pdf to a temp file that will be read in by gs
				file_put_contents($gsTemp, $pdfFile . PHP_EOL, FILE_APPEND);
			} else {
				errorHandle::errorMsg("PDF file not found: " . $pdfFile);
				throw new Exception("PDF file creation failed: " . $pdfFile);
			}
			...
			if (file_exists($htmlFile)) {
				...
				errorHandle::warningMsg("File not found for deletion: " . $htmlFile);
			}
		} else {
			errorHandle::warningMsg("File not found: " . $jpgFile);
		}
	}
