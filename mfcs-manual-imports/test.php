<?php
    ...
    //     sleep(1);
    // }    
    // $objectID = find('151', 'Women--Suffrage', 'subjectTopical');
    // // if objectID is null then create the object
    // if ($objectID === null) {
    //     $data = buildCreatorArray('151', 'Women--Suffrage');
    //     create('151', $data);
    // }

    // $objectID = find('151', 'Abercrombie, Neil', 'subjectTopical');
    $choicesFormID = findChoicesFormID('151', 'creator');
    // var_dump($choicesFormID);
    // get the $choicesFormID form
    $choices = objects::getAllObjectsForForm($choicesFormID);
    // var_dump($choices);

    // loop over each item in the $choices array
    foreach ($choices as $choice) {    
        // loop over each entry in the $choice['data'] array to find a match
        foreach ($choice['data'] as $key => $val) {
            if (trimAndCompare($val, 'Abercrombie, Neil')) {
                var_dump($choice['ID']);
            }
        }
    }
    
    // $objectID = find($choicesFormID, 'Abercrombie, Neil');
    // var_dump($objectID);
?>